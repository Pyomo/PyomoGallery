...
try:
    ...
    def build_heat_exchanger_results_doc(results, folders, append, iter_num, datafile_model, filename, instance, active_hx, old_points, errors, local_time, total_time, epsilons, tolerances, iteration):
        ...
        doc.append(NoEscape(r'\section{$\textsl{RecLMTD}^\beta$ Tangent Points}'))
        doc.append(NoEscape(r'\thispagestyle{plain}'))
        add_reclmtd_beta_results(instance, active_hx, errors, doc, level)
        generate_reclmtd_beta_graphs(instance, active_hx, old_points, doc, level)
        ...
        doc.append(NoEscape(r'\clearpage'))
        doc.append(NoEscape(r'\section{$\text{q}^\beta$ Breakpoints}'))
        doc.append(NoEscape(r'\thispagestyle{plain}'))
        add_q_beta_results(instance, active_hx, old_points, errors, doc, level)
        add_q_beta_graphs(instance, active_hx, old_points, doc, level)

        doc.append(NoEscape(r'\clearpage'))
        doc.append(NoEscape(r'\section{Area $q \beta$ Breakpoints}'))
        doc.append(NoEscape(r'\thispagestyle{plain}'))
