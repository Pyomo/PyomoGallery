...
def add_preamble(doc):
    ...
    # doc.preamble.append(NoEscape(r'\pgfplotsset{compat=1.10}'))
...
def add_summary(datafile_model, append, iter_num, tac, local_time, total_time, epsilons, tolerances, doc, level):
    ...
    # doc.append(NoEscape(r'%sRun name: %s\\' % append))
    ...
    level = level + 1
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\item $\varepsilon_{\textsl{LMTD}}$: %s' % (tab_gap, str(epsilons[reclmtd_ref]))))
    doc.append(NoEscape(r'%s\item $\varepsilon_{\textsl{Balancing}}$: %s' % (tab_gap, str(epsilons[balancing_ref]))))
    doc.append(NoEscape(r'%s\item $\varepsilon_{\textsl{Area}}$: %s' % (tab_gap, str(epsilons[area_ref]))))
    doc.append(NoEscape(r'%s\item $\varepsilon_{\textsl{Beta}}$: %s' % (tab_gap, str(epsilons[beta_ref]))))
    level = level - 1
    tab_gap = '\t'*level
    ...
    doc.append(NoEscape(r'%s%% hello' % (tab_gap)))
...
def generate_balancing_graphs(instance, active_hx, old_points, doc, level):
    ...
    for group in [th_points, thx_points, tc_points, tcx_points]:
        if group == th_points:
            index_extractor = lambda i,_,k: (i,k)
            temp_str = 'H'
            var = instance.th
            ...
            index_extractor = lambda _,j,k: (j,k+1)
            temp_str = 'C'
            var = instance.tc
...
def generate_reclmtd_list(reclmtd_value, abs_error, rel_error, doc, level):
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\begin{itemize}' % (tab_gap)))
    level += 1
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\item Value: %f' % (tab_gap, reclmtd_value)))
...
def add_reclmtd_results(instance, active_hx, errors, doc, level):
    ...
    tab_gap = '\t'*level
    for active in active_hx[stream_hx]:
        reclmtd_value = value(instance.reclmtd[active])
        abs_error = errors[stream_hx][reclmtd_error][active][absolute_error]
        rel_error = errors[stream_hx][reclmtd_error][active][relative_error]

        doc.append(NoEscape(r'%s\item $%s$' % (tab_gap, str(active))))
        generate_reclmtd_list(reclmtd_value, abs_error, rel_error, doc, level)

    for active in active_hx[cu_hx]:
        reclmtd_value = value(instance.reclmtd_cu[active])
        abs_error = errors[cu_hx][reclmtd_error][active][absolute_error]
        rel_error = errors[cu_hx][reclmtd_error][active][relative_error]

        doc.append(NoEscape(r'%s\item $\textit{cu},%s$' % (tab_gap, str(active))))
        generate_reclmtd_list(reclmtd_value, abs_error, rel_error, doc, level)

    for active in active_hx[hu_hx]:
        reclmtd_value = value(instance.reclmtd_hu[active])
        abs_error = errors[hu_hx][reclmtd_error][active][absolute_error]
        rel_error = errors[hu_hx][reclmtd_error][active][relative_error]
...
def add_reclmtd_tikz_preamble(minn, maxx, doc, level):
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s[' % (tab_gap)))
    level += 1
    tab_gap = '\t'*level
...
def generate_reclmtd_graphs(instance, active_hx, old_points, doc, level):
    tab_gap = '\t'*level
    iteration = 0
    for active in active_hx[stream_hx]:
        ...
        doc.append(NoEscape(r'%s\begin{axis}' % (tab_gap)))
        add_reclmtd_tikz_preamble(minn, maxx, doc, level)
...
def generate_area_list(area_value, abs_error, rel_error, doc, level):
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\begin{itemize}' % (tab_gap)))
    level +=1
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\item Value: %f' % (tab_gap, area_value)))
    doc.append(NoEscape(r'%s\item Error: %.15f' % (tab_gap, abs_error)))
    doc.append(NoEscape(r'%s\item Relative error: %.15f' % (tab_gap, rel_error)))
    level -=1
    tab_gap = '\t'*level
...
def add_area_results(instance, active_hx, old_points, errors, doc, level):
    tab_gap = '\t'*level
    beta = instance.Beta
    ...
    for group in [stream_hx, cu_hx, hu_hx]:
        if group == stream_hx:
            ...
            section_header = r'$A_{CU}$'
            index_template = r'\textit{cu},%s'
            var = instance.area_cu
        elif group == hu_hx:
            section_header = r'$A_{HU}$'
            index_template = r'\textit{hu},%s'
            var = instance.area_hu
        if len(active_hx[group]) != 0:
            ...
            for active in active_hx[group]:
                ...
                area_value = var[active].value
                abs_error = errors[group][area_error][active][absolute_error]
                rel_error = errors[group][area_error][active][relative_error]

                doc.append(NoEscape(r'%s\item $%s$' % (tab_gap, index_str)))
                generate_area_list(area_value, abs_error, rel_error, doc, level)
            level = level - 1
            tab_gap = '\t'*level
            doc.append(NoEscape(r'%s\end{itemize}' % (tab_gap)))

def add_area_graphs(instance, active_hx, old_points, doc, level):
    tab_gap = '\t'*level
    beta = instance.Beta
    iteration = 0
    for group in [stream_hx, cu_hx, hu_hx]:
        ...
        for active in active_hx[group]:
            ...
            breakpoints = old_points[group][q_points][active]
            x_min = 0
...
def generate_area_beta_list(area_value, area_beta, beta, breakpoints, abs_error, rel_error, doc, level):
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\begin{itemize}' % (tab_gap)))
    level +=1
    tab_gap = '\t'*level
    doc.append(NoEscape(r'%s\item $q$: %f' % (tab_gap, area_value)))
    doc.append(NoEscape(r'%s\item $\hat{q}^\beta$: %f' % (tab_gap, area_beta) ))
    doc.append(NoEscape(r'%s\item $q^\beta$: %f' % (tab_gap, pow(area_value, beta))))
...
def add_area_beta_results(instance, active_hx, old_points, errors, doc, level):
    ...
    for group in [stream_hx, cu_hx, hu_hx]:
        if group == stream_hx:
            index_template = r'$%s$'
            area_var = instance.area
            area_beta_var = instance.area_beta
        elif group == cu_hx:
            index_template = r'$\textit{cu},%s$'
            area_var = instance.area_cu
            area_beta_var = instance.area_cu_beta
        elif group == hu_hx:
            index_template = r'$\textit{hu},%s$'
            area_var = instance.area_hu
            area_beta_var = instance.area_hu_beta

        for active in active_hx[group]:
            index_str = index_template % (str(active))
            area_value = area_var[active].value
            area_beta = area_beta_var[active].value
            breakpoints = get_breakpoint_list(old_points[group][area_beta_points][active], area_value)
            abs_error = errors[group][area_beta_error][active][absolute_error]
            rel_error = errors[group][area_beta_error][active][relative_error]

            doc.append(NoEscape(r'%s\item %s' % (tab_gap, index_str)))
            generate_area_beta_list(area_value, area_beta, beta, breakpoints, abs_error, rel_error, doc, level)
...
def add_area_beta_tikz_main(beta, breakpoints, x_min, x_max, y_min, y_max, y_breakpoints, q, index_str, doc, level, iteration):
    tab_gap = '\t'*level
    if iteration % 2 == 0:
        doc.append(NoEscape(r'%s\begin{figure}[p]' % (tab_gap)))
        level += 1
    ...
    add_area_beta_tikz_preamble(x_min, x_max, y_min, y_max, doc, level)
    level += 1
    tab_gap = '\t'*level
...
def add_area_beta_graphs(instance, active_hx, old_points, doc, level):
    ...
    for group in [stream_hx, cu_hx, hu_hx]:
        ...
        if group == stream_hx:
            ...
            var = instance.area_hu
        for active in active_hx[group]:
            breakpoints = old_points[group][area_beta_points][active]
            x_min = 0
            x_max = breakpoints[len(breakpoints)-1]
            y_min = 0
            y_max = pow(x_max, beta)
            y_breakpoints = [pow(area, beta) for area in breakpoints]
            # y_breakpoints = map(lambda area: pow(area, beta), breakpoints)
            area = var[active].value
            index_str = index_template % (str(active))
            level = add_area_beta_tikz_main(beta, breakpoints, x_min, x_max, y_min, y_max, y_breakpoints, area, index_str, doc, level, iteration)
            iteration += 1
        if iteration % 2 == 1:
            level -= 1
            tab_gap = '\t'*level
...
def get_breakpoint_list(breakpoints, value):
    ...
    # string_breakpoints = map(lambda x: r'$%s$' % str(x), breakpoints)
